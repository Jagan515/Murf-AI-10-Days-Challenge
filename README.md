...
Challenge complete—all 10 days conquered! Check the daily folders for code, READMEs, and sample outputs. Each day builds on the starter, with updates to `agent.py` and more!
...
| **Day 10** | 01 December 2025 | Voice Improv Battle (Bollywood Edition) | Built single-player improv host "Raj Masala King Kapoor" with Hinglish, 3 desi scenarios/rounds, varied reactions, masala scores; persists sessions in `improv_data/`. Tested full improv battle and shared LinkedIn video. [Details → Day-10/README.md](Day-10/README.md) |

*Track progress via commits, JSON order logs (e.g., sample orders from Day 2 testing: `order_20251123_100209_317531.json` for a large cappuccino), wellness logs (e.g., `wellness_log.json` entries for Day 3 moods/goals), tutor courses (Day 4), lead captures (e.g., `lead_20251126_100000.json` for Day 5), fraud DB updates (e.g., `fraud_cases.json` with confirmed_safe/fraud entries for Day 6), grocery orders (e.g., `order_20251128_143022.json` for Day 7), game saves (e.g., `save_20251129_143022.json` with karma/state for Day 8), ecommerce orders (e.g., `orders.json` with ORD- entries for Day 9), improv results (e.g., `bollywood_game_20251201_143022.json` with masala scores/notes for Day 10), and daily videos on LinkedIn. All days build cumulatively—run the full stack for the latest agent!*
...
├── Day-8/                  # Day 8: Voice Game Master (D&D-Style Adventure)
│   ├── backend/            # Updated Python agent (includes src/agent.py with GM tools, game_saves/ JSON world/state)
│   │   └── game_saves/     # Sample game JSONs (e.g., world_setup.json, save_20251129_143022.json with karma/events)
│   ├── frontend/           # Updated Next.js UI (Jungle-themed)
...
├── Day-10/                 # Day 10: Voice Improv Battle (Bollywood Edition)
│   ├── backend/            # Updated Python agent (includes src/agent.py with improv tools, improv_data/ scenarios/saves)
│   │   └── improv_data/    # Sample improv JSONs (e.g., bollywood_scenarios.json, bollywood_game_20251201_143022.json with masala scores)
│   ├── frontend/           # Updated Next.js UI (Bollywood-themed)
│   ├── LICENSE             # MIT License for Day 10
│   ├── README.md           # Day 10 details and setup
│   └── start_app.sh        # Convenience script for Day 10
...
```bash
cd Day-*/backend  # e.g., Day-10/backend
...
```bash
cd Day-*/frontend  # e.g., Day-10/frontend
# Install dependencies
pnpm install
# Copy environment file and configure
cp .env.example .env.local
...
**Challenge complete—relive the 10-day journey!** See `Day-2/challenges/` for official MDs.
...
Challenge complete—relive the journey! ☕🎙️

---

*Built for the AI Voice Agents Challenge by murf.ai. My fork updated on December 01, 2025.*